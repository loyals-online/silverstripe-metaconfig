...
class MetaConfigPageExtension extends DataExtension
{
    ...
    /**
     * Hook into the onBeforePageInit extend call
     *
     */
    public function onBeforePageInit()
    {
        if ($this->isCMS()) {
            return;
        }
    ...
    /**
     * Insert Google Tag Manager or Google Analytics script if their IDs are filled
     *
     * @note    Tag Manager has priority over Analytics
     */
    protected function loadGoogleScripts()
    {
        if ($this->getSiteConfig()->GoogleTagManagerID) {
            Requirements::insertHeadTags("
                <script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});
                var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';
                j.async=true;j.src='//www.googletagmanager.com/gtm.js?id='+i+dl;
                f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','" . $this->getSiteConfig()->GoogleTagManagerID . "');</script>
            ");
        } elseif ($this->getSiteConfig()->AnalyticsID) {
            Requirements::insertHeadTags("
                <script>
                    ...
                    ga('create', '" . $this->getSiteConfig()->AnalyticsID . "', 'auto');
